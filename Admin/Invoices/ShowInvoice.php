<?php
    ...
    if (mysqli_num_rows($result) > 0) {
        foreach ($result as $index => $each) {
            $index += 1;
            if ($each['customerID'] != null) {
                $stringSQL = "SELECT * FROM `customers` WHERE `customerID` = '" . $each['customerID'] . "'";
                $customer = mysqli_fetch_array(mysqli_query($connect, $stringSQL));
                
                $each['name'] = $customer['name'];
            } else {
                $each['name'] = "Khách vãng lai";
            }
            echo "<tr>";
                ...
                echo "<td class='text-center' style='width: 10%'>
                        <span>" . $each['ma_hoadon'] . "</span>
                </td>";
                echo "<td class='text-center' style='width: 25%'>
                        <span>" . ($each['name']) . "</span>";
                    if ($each['customerID'] != null && $customer['phone'] != null && $customer['email'] != null) {
                        echo "<br><span class='text-muted'>" . ($customer['phone']) . " - " . ($customer['email']) . "</span>";
                    } elseif ($each['customerID'] != null && $customer['phone'] != null) {
                        echo "<br><span class='text-muted'>" . ($customer['phone']) . "</span>";
                    } elseif ($each['customerID'] != null && $customer['email'] != null) {
                        echo "<br><span class='text-muted'>" . ($customer['email']) . "</span>";
                    } elseif($each['customerID'] != null) {
                        echo "<br><span class='text-muted'>Không có thông tin</span>";
                    }
                echo "</td>";
                echo "<td class='text-center' style='width: 10%'>
                        <span>" . product_price($each['tienthanhtoan']) . "</span>
                </td>";
                echo "<td class='text-center' style='width: 10%'>
                        <span>" . product_price($each['phivc']) . "</span>
                </td>";
                if ($each['trangthai'] == 0) {
                    echo "<td class='text-center' style='width: 10%'>
                            <span class='text-warning'>Chờ xác nhận</span>
                        </td>";
                } elseif ($each['trangthai'] == 1) {
                    echo "<td class='text-center' style='width: 10%'>
                            <span class='text-primary'>Đã xác nhận, đang vận chuyển</span>
                        </td>";
                }
                        ...
                        <div class='dropdown'>
                            ...
                            <ul class='dropdown-menu'>
                                <li class='d-flex align-items-center'>
                                    <a href='InvoiceDetail.php?id=" . $each['ma_hoadon'] . "' class='dropdown-item d-flex align-items-center fs-5'>
                                        <ion-icon name='eye-outline'></ion-icon>
                                        <span class='ms-4'>Xem chi tiết</spam>
                                    </a>
                                </li>
                                <li class='d-flex align-items-center'>
                                    <a href='ApproveInvoice.php?id=" . $each['ma_hoadon'] . "' class='dropdown-item d-flex align-items-center fs-5'>
                                        <ion-icon name='checkmark-done-outline'></ion-icon>
                                        <span class='ms-4'>Duyệt</spam>
                                    </a>
                                </li>
                                <li class='d-flex align-items-center'>
                                    <a href='DeleteInvoice.php?id=" . $each['ma_hoadon'] . "' class='dropdown-item d-flex align-items-center fs-5'>
        ...
        echo "<tr>
                <td colspan='7' class='text-center'>Không có hoá đơn nào</td>
            </tr>";
    }