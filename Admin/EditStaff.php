...
<div class="modal fade show" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-modal="true" style="display: block; background: rgb(0,0,0,0.5);">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <form action="process_update_staff.php?id=<?php echo $each['id'];?>" method="post" class="form-add" style="height: 100%; width: 100%;">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-2" id="addleModalLabel">Sửa nhân viên</h1>
                    <a href="StaffCategory.php">X</a>
                </div>

                <div class="modal-body" style="overflow-x: hidden;">
                    <label class="fs-5 form-label required" for="nameStaff">1. Họ tên: </label>
                    <br>
                    <input id="nameStaff" name="name_staff" value="<?php echo $each['name'];?>" class="form-control form-control-lg" type="text" placeholder="Tên nhân viên..." aria-label=".form-control-lg" required>
                    <div class="invalid name_staff"></div>
                    <br>

                    <label class="fs-5 form-label required" for="phoneStaff">2. Số điện thoại: </label>
                    <br>
                    <input onchange="changeInput(event)" value="<?php echo $each['phone'];?>" id="phoneStaff" name="phone_staff" class="form-control form-control-lg" type="text" placeholder="Số điện thoại nhân viên..." aria-label=".form-control-lg" required>
                    <div class="invalid phone_staff"></div>
                    <br>

                    <label class="fs-5 form-label required" for="dobStaff">3. Ngày sinh: </label>
                    <br>
                    <input onchange="changeInputDate(event)" value="<?php echo $each['dob'];?>" id="dobStaff" name="dob_staff" class="form-control form-control-lg" type="date" required>
                    <div class="invalid dob_staff"></div>
                    <br>

                    <label class="fs-5 form-label required" for="addressStaff">4. Địa chỉ: </label>
                    <br>
                    <input id="addressStaff" name="address_staff" value="<?php echo $each['address'];?>" class="form-control form-control-lg" type="text" placeholder="Địa chỉ nhân viên..." aria-label=".form-control-lg" required>
                ...
                <?php if ($_SESSION['user']['role'] == 0 || $_SESSION['user']['role'] == 1) {?>
                    ...
                    <select name="role_staff" id="roleStaff" class="form-select" aria-label="Default select example">
                        <option value="1" <?php if ($each['role'] == 1) echo 'selected'; ?>>Quản lý</option>
                        <option value="2" <?php if ($each['role'] == 2) echo 'selected'; ?>>Nhân viên</option>
                    </select>
                    <br>
                <?php } ?>
                </div>
